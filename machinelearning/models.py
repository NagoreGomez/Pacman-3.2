...
class DigitClassificationModel(object):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"            # CON ESTOS PARAMETROS 21 EPOCH (CREO) Y ACCURACY=97.7
        #salida_tamaño=1         # definida arriba, output size
        entrada_tamaño=784        
        oculta_tamaño=700        
        self.batch_size = 400    # tamaño del lote (1-tamaño data)
    ...
    def run(self, x):
        ...
        entrada = nn.Linear(x, self.layers[0])          # X*W0 (cuando numero_ocultas=1)
        entrada = nn.AddBias(entrada, self.layers[1])   # (X*W0)+B0
        entrada = nn.ReLU(entrada)                      # relu(X*W0+B0)
